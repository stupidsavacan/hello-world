...
const IMAGE_MIME_TYPES: Record<string, string> = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp' };
...
function zipLookupPath(path: string): string { return path.replace(/\\/g, '/'); }
async function readReferencedAssets(zip: JSZip, pack: LoopDeckPack, issues: PackValidationIssue[]): Promise<ImportedPackAsset[]> {
  const assets: ImportedPackAsset[] = []; const referencedPaths = new Set(pack.questions.map((q) => q.imageAsset).filter((path): path is string => Boolean(path)));
  for (const path of referencedPaths) {
    if (!isSafeImageAssetRef(path)) { issues.push({ level: 'warning', message: 'Unsafe or unsupported image asset reference was not imported.', path }); continue; }
    const zipFile = zip.file(zipLookupPath(path)); if (!zipFile || zipFile.dir) { issues.push({ level: 'warning', message: 'Referenced image asset was not found in the ZIP.', path }); continue; }
    const mimeType = IMAGE_MIME_TYPES[extensionOf(path)]; if (!mimeType) { issues.push({ level: 'warning', message: 'Referenced image asset type is not supported.', path }); continue; }
    const base64 = await zipFile.async('base64'); assets.push({ packId: pack.packId, path, mimeType, dataUrl: `data:${mimeType};base64,${base64}` });
  }
  return assets;
}
export async function importLoopDeckZip(file: File): Promise<PackValidationResult> {
  ...
  const assets = await readReferencedAssets(zip, packResult.pack, issues); stageImportedPackAssets(packResult.pack, assets);
