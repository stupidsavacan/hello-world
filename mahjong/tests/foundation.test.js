...
if (missingRetirementGuards.length) throw new Error(`retirement guards disappeared: ${missingRetirementGuards.join(', ')}`);
...
['../loopdeck/foreign/mahjong/RuleConfig.js','../loopdeck/foreign/mahjong/Tile.js','../loopdeck/foreign/mahjong/SeedPolicy.js','src/game/Wall.js','../loopdeck/foreign/mahjong/Player.js'].forEach(load);
...
assert(wallA.tiles.map((tile) => tile.instanceId).join(',') === wallB.tiles.map((tile) => tile.instanceId).join(','), 'same seed must produce deterministic wall order');
...
const first = wallA.draw(); player.receiveTile(first);
...
let rejected = false;
try { new Sanma.Wall(config, { seed: 'x'.repeat(authority.maxSeedLength + 1) }); } catch (error) { rejected = new RegExp(String(authority.maxSeedLength)).test(error.message); }
